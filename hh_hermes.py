...
def _patch_dashboard_update_gate() -> None:
    ...
    *supposed* to disable the dashboard's built-in "Update Hermes" button
    ...
    It identifies that layout with ``Path("/opt/data").resolve() ==
    Path("/opt/data")``, which Hermes assumes is a no-op for a normal
    Docker bind mount. Modal instead mounts Volumes as an actual symlink
    to an internal ``/__modal/volumes/<id>`` path, so ``.resolve()``
    unwinds it and the comparison silently fails -- the dashboard reports
    ...
    in ``hermes_cli/web_server.py`` to keep Hermes itself unmodified.
    """
    ...
    try:
        ...
        return
    _ws._dashboard_local_update_managed_externally = lambda: True
